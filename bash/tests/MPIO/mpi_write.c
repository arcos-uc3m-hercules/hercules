...
int main( int argc, char *argv[] )
{
    ...
    // rc = MPI_File_open( comm, "/mnt/hercules/test.ord", MPI_MODE_RDWR | MPI_MODE_CREATE, MPI_INFO_NULL, &fh );
    rc = MPI_File_open( comm, "/mnt/hercules/test.ord",MPI_MODE_WRONLY | MPI_MODE_CREATE, MPI_INFO_NULL, &fh );
    //rc = MPI_File_open( comm, "test.ord", MPI_MODE_WRONLY | MPI_MODE_CREATE, MPI_INFO_NULL, &fh );
    if(rc) {
	    fprintf(stderr,"Unable to open file, rc=%d\n", rc);
	    MPI_Abort(comm, 911);
    }
