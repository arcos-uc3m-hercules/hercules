...
first_it=1
# set 0 in active nodes to stop all servers when test finish. This value is different to 0 when we use malleability.
...
SOURCE=/lustre/scratch/javier.garciablas/hercules/malleability/source/
SINK=/lustre/scratch/javier.garciablas/hercules/malleability/sink/$DIRECTORY_NAME_$num_data_servers
...
echo "cp -r $SOURCE$DIRECTORY_NAME /mnt/hercules/"
...
/usr/bin/time -f "[HS] Hercules save %E sec." cp -r /mnt/hercules/$DIRECTORY_NAME $SINK
unset LD_PRELOAD
unset HERCULES_CONF
