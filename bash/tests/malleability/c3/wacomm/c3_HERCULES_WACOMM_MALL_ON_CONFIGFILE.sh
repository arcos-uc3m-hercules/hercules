...
HERCULES_MALLEABILITY_THRESHOLD=5000 # MB
			...
			for NODES_FOR_CLIENTS in "${NODES_FOR_CLIENTS_RANGE[@]}"; do
				for CLIENTS_PER_NODE in "${CLIENTS_PER_NODE_RANGE[@]}"; do
					for BLOCK_SIZE in "${BLOCK_SIZE_RANGE[@]}"; do
						...
						sed -i "s|^HERCULES_MALLEABILITY_THRESHOLD = .*|HERCULES_MALLEABILITY_THRESHOLD = $HERCULES_MALLEABILITY_THRESHOLD|g" "$TEMPLATE_CONFIG_PATH"
