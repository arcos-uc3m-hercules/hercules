...
# Malleability params.
MALLEABILITY=1
#LOWER_BOUND_MALLEABILITY=( 2 4 8 16 )
LOWER_BOUND_MALLEABILITY=( 2 )
...
do	
	CURR_UPPER_BOUND_MALLEABILITY=$NUM_SERVERS
	for CURR_LOWER_BOUND_MALLEABILITY in "${LOWER_BOUND_MALLEABILITY[@]}"
	do
		...
		do
			...
			do
				...
				do
					...
					CONFIG_PATH="../conf/${NUM_SERVERS}s-${NODES_FOR_CLIENTS}nfc-${CLIENTS_PER_NODE}cpn_${BLOCK_SIZE}blocksize_${MSG_}_MALL.conf"
					...
					if ! [ -f "$CONFIG_PATH" ]; then
						...
						## Search the labels and replace with the current value in the temporal configuration file.
						...
						sed -i "s/^UPPER_BOUND_MALLEABILITY = [0-9]*/UPPER_BOUND_MALLEABILITY = $CURR_UPPER_BOUND_MALLEABILITY/g"  "$TEMPLATE_CONFIG_PATH"
						sed -i "s/^LOWER_BOUND_MALLEABILITY = [0-9]*/LOWER_BOUND_MALLEABILITY = $CURR_LOWER_BOUND_MALLEABILITY/g"  "$TEMPLATE_CONFIG_PATH"
						## Creates the final conf. file for this deployment.
						cat $TEMPLATE_CONFIG_PATH > "$CONFIG_PATH"
						## Copy the temporal conf. file to the template one.
