...
int main(int argc, char **argv)
{
     ...
     */
    fprintf(stderr, "Running set view\n");
    err = MPI_File_set_view(fh, offset, MPI_BYTE, MPI_BYTE, "native", MPI_INFO_NULL);
    fprintf(stderr, "Ending set view\n");
    // err = MPI_File_set_view(fh, offset, MPI_INT, MPI_INT, "native", MPI_INFO_NULL);
    CHECK_ERR(MPI_File_set_view);

    offset += offset;

    fprintf(stderr, "Running set view 2\n");
    err = MPI_File_set_view(fh, offset, MPI_BYTE, MPI_BYTE, "native", MPI_INFO_NULL);
    fprintf(stderr, "Ending set view 2\n");
    // err = MPI_File_set_view(fh, offset, MPI_INT, MPI_INT, "native", MPI_INFO_NULL);
    CHECK_ERR(MPI_File_set_view);
     ...
     */
    err = MPI_File_write_all(fh, &buf[0], 3, MPI_BYTE, &status);
     ...
     * write call.
     */
    err = MPI_File_write_all(fh, &buf[3], 7, MPI_BYTE, &status);
    CHECK_ERR(MPI_File_write_all);
