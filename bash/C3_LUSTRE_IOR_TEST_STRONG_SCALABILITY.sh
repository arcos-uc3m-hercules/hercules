...
SCRIPT_NAME="c3_ior_lustre_slurm.sh"
...
do
	...
	do
		...
		do
			...
			if [ "$jid" -eq 1 ]; then
				jid=$(sbatch -A $account -N $NUMBER_OF_NODES $SCRIPT_NAME "$FILE_SIZE_PER_CLIENT" "$TOTAL_NUMBER_OF_CLIENTS" "$PROCESS_PER_NODE" "$IOR_FILE_PER_PROCESS" "$IOR_AVOID_CACHE" | cut -d ' ' -f4)
				echo $jid
			## The following jobs wait for the previous job to finish.
			else
				jid=$(sbatch -A $account --dependency=afterany:"${jid}" -N $NUMBER_OF_NODES $SCRIPT_NAME "$FILE_SIZE_PER_CLIENT" "$TOTAL_NUMBER_OF_CLIENTS" "$PROCESS_PER_NODE" "$IOR_FILE_PER_PROCESS" "$IOR_AVOID_CACHE" | cut -d ' ' -f4)
				echo $jid
			fi
			set +x
