...
SBATCH_FLAGS="--partition=large -A ${SLURM_ACC} --exclusive" 
...
#TEST_TYPE="weak"
TEST_TYPE="strong"
HERCULES_PATH="/home/user/hercules/"
...
MALLEABILITY=0 # disable, 1 to enable
...
#NUM_DATA_SERVERS_RANGE=( 1 2 4 8 16 32 )
NUM_METADATA_SERVERS_RANGE=(1)
#NUM_METADATA_SERVERS_RANGE=( 1 2 4 8 16 )
	...
	for METADATA_NUM_SERVERS in "${NUM_METADATA_SERVERS_RANGE[@]}"; do
		for DATA_NUM_SERVERS in "${NUM_DATA_SERVERS_RANGE[@]}"; do
			if [ $MALLEABILITY -eq 0 ]; then
				MAX_DATA_SERVERS_RANGE=${DATA_NUM_SERVERS}
			fi
			for NODES_FOR_CLIENTS in "${NODES_FOR_CLIENTS_RANGE[@]}"; do
				for CLIENTS_PER_NODE in "${CLIENTS_PER_NODE_RANGE[@]}"; do
					for BLOCK_SIZE in "${BLOCK_SIZE_RANGE[@]}"; do
						...
						# Makes a temporal configuration file to write the values for this test.
