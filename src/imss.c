...
uint32_t n_stat_servers = 1; // Number of metadata servers available.
...
// dataset_info *root_directory_info = NULL;
imss empty_imss;
...
{
	return g_int_hash(GINT_TO_POINTER(*(int *)key));
}

// Define a custom equality function for integer pointers
gboolean int_pointer_equal(gconstpointer a, gconstpointer b)
{
	return g_int_equal(GINT_TO_POINTER(*(int *)a), GINT_TO_POINTER(*(int *)b));
}

// Define a destroy function to free the allocated memory
...
/**
 * @brief Callback function which releases resources of a dataset when calling the "remove"
 * method of a ghashtable.
 * @param data pointer to the dataset to be freed.
 */
...
/**
 * @brief Callback function which frees the memory allocated for an entry on the ghashtable.
 * @param data pointer to the data to be freed.
 */
void free_ghashtable_entry(gpointer data)
{
	if (data)
	{
		g_free(data);
...
gboolean replace_uri_base_path_dir(GHashTable *hash_table, const char *old_base_uri, const char *new_base_uri)
{
	...
	{
		slog_warn("Invalid input to replace_uri_base_path_dir.");
		return FALSE;
	}
	...
	size_t len_old_base_uri = strlen(old_base_uri);
	size_t len_current_key = 0;
	while (g_hash_table_iter_next(&iter, &current_key, &current_value))
	{
		slog_debug("Comparing %s - %s", old_base_uri, current_key);
		const char *curr_key_str = (const char *)current_key;
		// Check if the current key matches the old base URI prefix.
		if (g_str_has_prefix(curr_key_str, old_base_uri))
		{
			len_current_key = strlen(curr_key_str);
			// check if current_key is a child of the old uri.
			// for example, imss://dir10 is not a child of imss://dir1, but
			// imss://dir1 is a substring (prefix) of imss://dir10.
			if (len_old_base_uri < len_current_key)
			{
				if (curr_key_str[len_old_base_uri] != '/')
				{ // current_key is not a child of old_base_uri.
					slog_debug("Skipping %s", curr_key_str);
					continue;
				}
			}
			slog_debug("  Found matching key: '%s'", current_key);
			keys_to_replace = g_list_append(keys_to_replace, g_strdup(curr_key_str));
		}
	}
	...
	if (!modified_any)
	{
		slog_debug("No entries found with prefix '%s' to modify.", old_base_uri);
	}
...
int32_t find_imss_pointer(const char *imss_uri, imss **out_ptr)
{
	...
	for (int32_t i = 0; i < imssd->len; i++)
	{
		imss *current_element = &g_array_index(imssd, imss, i);

		if (!strncmp(imss_uri, current_element->info.uri_, URI_))
		{
			if (out_ptr != NULL)
			{
...
/**
 * @brief Helper to clean up allocated metadata resources safely on error.
 *
 */
static void free_metadata_resources(FILE *stat_nodes_struct, char *stat_node, int servers)
{
	if (stat_nodes_struct)
	{
		fclose(stat_nodes_struct);
	}
	if (stat_node)
	{
		free(stat_node);
...
static int init_glib_structures(void)
{
...
static int resolve_network_interfaces(void)
{
	...
	for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next)
	{
		...
		if (family == AF_INET)
		{
			...
			{
				slog_debug("Skip - name: %s, ip: %s", ifa->ifa_name, host);
				continue;
			}

			slog_debug("Select - name: %s, ip: %s", ifa->ifa_name, host);
...
static int connect_to_metadata_servers(const char *stat_hostfile, uint64_t port, uint32_t rank)
{
	FILE *stat_nodes_struct = NULL;
	char *stat_node = NULL;
	size_t l_size = 0; // LINE_LENGTH;
	...
	stat_eps = (ucp_ep_h *)calloc(n_stat_servers, sizeof(ucp_ep_h));
	ep_contexts = (client_ep_context_t **)calloc(n_stat_servers, sizeof(client_ep_context_t *));
	stat_addr = (ucp_address_t **)calloc(n_stat_servers, sizeof(ucp_address_t *));

	if (!stat_ids || !stat_eps || !ep_contexts || !stat_addr)
	{
		perror("HERCULES_ERR_STAT_INIT_MEMORY_ALLOC");
		slog_error("HERCULES_ERR_STAT_INIT_MEMORY_ALLOC");
		free_metadata_resources(stat_nodes_struct, stat_node, n_stat_servers);
		return -1;
	}
	...
	ucs_status_t status = ucp_worker_query(ucp_worker_meta, &worker_attr);
	if (status != UCS_OK)
	{
		free_metadata_resources(stat_nodes_struct, stat_node, n_stat_servers);
		return -1;
	...
	for (int i = 0; i < n_stat_servers; i++)
	{
		...
		int n_chars = getline(&stat_node, &l_size, stat_nodes_struct);
		if (n_chars <= 0)
		{
		...
		slog_debug("[IMSS] i=%d, stat_node=%s, port=%ld, rank=%" PRIu32 "", i, stat_node, port, rank);
		slog_debug("[IMSS] Contacting stat dispatcher at %s:%ld", stat_node, port);

		int oob_sock = connect_common(stat_node, port, AF_INET);
		if (oob_sock < 0)
		{
			char err_msg[MAX_ERR_MSG_LEN];
			snprintf(err_msg, sizeof(err_msg), "HERCULES_ERR_CONNECT-%s:%ld", stat_node, port);
			perror(err_msg);
			pthread_mutex_unlock(&lock_network);
			free_metadata_resources(stat_nodes_struct, stat_node, n_stat_servers);
			return -1;
		...
		snprintf(request, REQUEST_SIZE, "%" PRIu32 " GET HELLO!", rank);
		slog_debug("[IMSS] request=%s", request);

		if (send(oob_sock, request, REQUEST_SIZE, 0) < 0)
		{
			char err_msg[MAX_ERR_MSG_LEN];
			snprintf(err_msg, sizeof(err_msg), "HERCULES_ERR_STAT_HELLO_2-%s:%ld", stat_node, port);
			close(oob_sock);
			pthread_mutex_unlock(&lock_network);
			free_metadata_resources(stat_nodes_struct, stat_node, n_stat_servers);
			return -1;
		...
		if (recv(oob_sock, &addr_len, sizeof(addr_len), MSG_WAITALL) < 0)
		{
		...
		stat_addr[i] = (ucp_address_t *)malloc(addr_len);
		if (stat_addr[i] == NULL)
		{
		...
		if (recv(oob_sock, stat_addr[i], addr_len, MSG_WAITALL) < 0)
		{
		...
		status = client_create_ep_data_with_context(ucp_worker_meta, &stat_eps[i], stat_addr[i], metadata_err_call_arg, &ep_contexts[i]);
		if (status != UCS_OK)
		{
			slog_error("HERCULES_ERR_CLIENT_CREATE_EP_DATA");
			fprintf(stderr, "HERCULES_ERR_CLIENT_CREATE_EP_DATA");
			pthread_mutex_unlock(&lock_network);
			free_metadata_resources(stat_nodes_struct, stat_node, n_stat_servers);
			return -1;
		}
	...
	if (fclose(stat_nodes_struct) != 0)
	{
		...
		pthread_mutex_unlock(&lock_network);
		free_metadata_resources(NULL, NULL, n_stat_servers);
		return -1;
	}
...
/**
 * @brief Method creating a communication channel with the IMSS metadata server. Besides, the stat_imss method initializes a set of elements that will be used through the session.
 *
 */
int32_t stat_init(char *stat_hostfile, uint64_t port, int32_t num_stat_servers, uint32_t rank)
{
	...
	if (init_glib_structures() != 0)
	{
		return -1;
	}

	if (resolve_network_interfaces() != 0)
	{
		return -1;
	}

	if (connect_to_metadata_servers(stat_hostfile, port, rank) != 0)
	{
		return -1;
	}
...
{
	ucp_ep_h ep;
	ep = stat_eps[server_id];
	if (send_req(ucp_worker_meta, ep, local_addr_meta, local_addr_len_meta, message) == 0)
	{
		slog_error("HERCULES_ERR_STATACK_SEND_REQ");
		perror("HERCULES_ERR_STATACK_SEND_REQ");
		return -1;
	}
	return 0;
}
...
int32_t stat_release()
{
	...
	sprintf(release_msg_metadata, "%" PRIu32 " GET 2 RELEASE", process_rank);
	for (int32_t i = 0; i < n_stat_servers; i++)
	{
		ep = stat_eps[i];
		flush_ep(ucp_worker_meta, ep);
		slog_live("Request - %s", release_msg_metadata);
		// request to close the endpoint on the server side.
		if (send_req(ucp_worker_meta, ep, local_addr_meta, local_addr_len_meta, release_msg_metadata) == 0)
		{
...
uint32_t get_dir(std::string requested_uri_obj, char ***items)
{
	...
	char *arr_elements[number_metadata_servers]; //= (char **)malloc(number_metadata_servers * sizeof(char *));
	...
	pthread_mutex_lock(&lock_network);
	size_t len_requested_uri = strlen(requested_uri);
	...
	for (int i = m_srv; i < m_srv + number_metadata_servers; i++)
	{
		...
		// GETDIR request.
		// Concat the last slash to tell the metadata server this uri is from a directory.

		if (requested_uri[len_requested_uri - 1] == '/')
		{
		...
		slog_debug("Num. elements from metadata server %d", num_elements);
	...
	*items = (char **)calloc(total_num_elements, sizeof(char *));
	...
	slog_debug("Copying root %s to items", requested_uri);
	memcpy((*items)[0], requested_uri, strlen(requested_uri));
...
/**
 * @brief Method initializing the required resources to make use of an existing HERCULES instance.
 * @param imss_uri URI assigned to the HERCULES instance that the client desires to connect.
 * @return
 *  On success, the number of active data storage servers is returned.
...
int32_t open_imss(char *imss_uri, uint32_t *num_active_storages)
{
	...
	for (int32_t i = 0; i < new_imss.info.num_storages; i++)
	{
		...
		{
			char err_msg[MAX_ERR_MSG_LEN];
			sprintf(err_msg, "HERCULES_ERR_OPEN_IMSS_CONNECT_COMMON - i=%d - %s:%" PRIu16, i, new_imss.info.ips[i], new_imss.info.conn_port);
			...
			free(new_imss.conns.ep_contexts);
			return -1;
		}

		char request[REQUEST_SIZE] = {0};
		sprintf(request, "%" PRIu32 " GET %s", process_rank, "HELLO!JOIN");
		slog_live("ip_address=%s:%" PRIu16 ", local_data_uid=%d", new_imss.info.ips[i], new_imss.info.conn_port, local_data_uid);
...
int32_t AddBackEndServer2Imss(imss *local_imss_, int at_position)
{
	...
	uint32_t *temp_id = NULL;
	client_ep_context_t **temp_ep_contexts = NULL;

	slog_debug("[IMSS] session_policy local_imss_->info.num_active_storages=%ld, num_storages=%ld", local_imss_->info.num_active_storages, local_imss_->info.num_storages);

	// -1 means append, otherwise insert at the given slot
	int i = (at_position < 0 || at_position >= (int)local_imss_->info.num_storages)
		    ? (int)local_imss_->info.num_storages - 1
		    : at_position;

	// Reallocate peer_addr
	temp_peer_addr = (ucp_address_t **)realloc(local_imss_->conns.peer_addr,
						   local_imss_->info.num_storages * sizeof(*local_imss_->conns.peer_addr));
	if (temp_peer_addr == NULL)
	{
		slog_error("HERCULES_ERR_REALLOC_FAILED: Failed to reallocate temp_peer_addr");
		return -1;
	}
	local_imss_->conns.peer_addr = temp_peer_addr;

	// Reallocate endpoints (eps)
	temp_eps = (ucp_ep_h *)realloc(local_imss_->conns.eps,
				       local_imss_->info.num_storages * sizeof(ucp_ep_h));
	if (temp_eps == NULL)
	{
		slog_error("HERCULES_ERR_REALLOC_FAILED: Failed to reallocate temp_eps");
		return -1;
	}
	local_imss_->conns.eps = temp_eps;

	// Reallocate IDs
	temp_id = (uint32_t *)realloc(local_imss_->conns.id,
				      local_imss_->info.num_storages * sizeof(uint32_t));
	if (temp_id == NULL)
	{
		slog_error("HERCULES_ERR_REALLOC_FAILED: Failed to reallocate temp_id");
		return -1;
	}
	local_imss_->conns.id = temp_id;

	// Reallocate endpoint contexts
	temp_ep_contexts = (client_ep_context_t **)realloc(local_imss_->conns.ep_contexts,
							   local_imss_->info.num_storages * sizeof(client_ep_context_t *));
	if (temp_ep_contexts == NULL)
	{
		slog_error("HERCULES_ERR_REALLOC_FAILED: Failed to reallocate temp_ep_contexts");
		return -1;
	}
	local_imss_->conns.ep_contexts = temp_ep_contexts;

	// Shift connection arrays right from the end down to position i
	int last = (int)local_imss_->info.num_storages - 1;
	for (int k = last; k > i; k--)
	{
		local_imss_->conns.peer_addr[k] = local_imss_->conns.peer_addr[k - 1];
		local_imss_->conns.eps[k] = local_imss_->conns.eps[k - 1];
		local_imss_->conns.id[k] = local_imss_->conns.id[k - 1];
		local_imss_->conns.ep_contexts[k] = local_imss_->conns.ep_contexts[k - 1];
	}

	local_imss_->conns.ep_contexts[i] = NULL;
	local_imss_->conns.peer_addr[i] = NULL;
	local_imss_->conns.eps[i] = NULL;
	...
	if (send(oob_sock, request, REQUEST_SIZE, 0) < 0)
	{
		slog_error("HERCULES_ERR_IMSS_OPEN_IMSS_SEND_REQUEST: Failed to send JOIN request");
		close(oob_sock);
		return -1;
	}
	...
	if (ret < 0)
	{
		slog_error("HERCULES_ERR_IMSS_OPEN_IMSS_RECV_ADDR_LEN: Failed to receive address length");
		close(oob_sock);
		return -1;
	}

	local_imss_->conns.peer_addr[i] = (ucp_address *)malloc(addr_len);
	if (local_imss_->conns.peer_addr[i] == NULL)
	{
		slog_error("HERCULES_ERR_IMSS_MALLOC_FAILED: Failed to allocate peer_addr memory");
		close(oob_sock);
		return -1;
	}

	ret = recv(oob_sock, local_imss_->conns.peer_addr[i], addr_len, MSG_WAITALL);
	if (ret < 0)
	{
		slog_error("HERCULES_ERR_IMSS_OPEN_IMSS_RECV_PEER_ADDR: Failed to receive peer address");
		close(oob_sock);
		return -1;
	}
	close(oob_sock);

	// re-sequence all IDs to match their array positions after the shift
	for (int k = i; k < (int)local_imss_->info.num_storages; k++)
		local_imss_->conns.id[k] = k;

	// Check if the new server matches the local node/IP
	if (!strncmp((local_imss_->info.ips)[i], client_node, len_client_node) ||
	    !strncmp((local_imss_->info.ips)[i], client_ip, strlen(local_imss_->info.ips[i])))
	{
		local_imss_->conns.matching_server = i;
		matching_server_id = i;
	}
	...
	// Update the IMSS structure in the global registry
	// g_array_remove_index(imssd, curr_dataset->imss_d);
	// g_array_insert_val(imssd, curr_dataset->imss_d, curr_imss);

	return local_imss_->info.num_storages;
}
...
int32_t release_network_resources(const char *imss_uri, int is_parent, int process_rank)
{
	...
	// Release the set of connections to the corresponding IMSS.
	slog_live("imss_position=%d, num_active_storages=%d, num_storages=%d", imss_position, imss_.info.num_active_storages, imss_.info.num_storages);
	ucp_ep_h ep;
	for (int32_t i = 0; i < imss_.info.num_storages; i++)
	{
	...
	for (int32_t i = 0; i < imss_.info.num_storages; i++)
	{
		...
		free(imss_.info.ips[i]);
		free(imss_.conns.peer_addr[i]);
		imss_.info.ips[i] = NULL;
		imss_.conns.peer_addr[i] = NULL;
	}
	free(imss_.conns.eps);
	free(imss_.info.ips);
	free(imss_.conns.peer_addr);
	imss_.conns.eps = NULL;
	imss_.info.ips = NULL;
	imss_.conns.peer_addr = NULL;

	// Free UCX resources with the metadata server.
	char release_msg_metadata[REQUEST_SIZE] = {0};

	snprintf(release_msg_metadata, REQUEST_SIZE, "%" PRIu32 " GET 2 RELEASE", process_rank);
	...
	// flush the worker
	status = ucp_worker_flush(ucp_worker_meta);
	if (status != UCS_OK)
	{
		fprintf(stderr, "Failed to flush worker: %s\n", ucs_status_string(status));
	}
...
int32_t release_imss(const char *imss_uri, uint32_t release_op)
{
	...
	if ((imss_position = find_imss(imss_uri, &imss_)) == -1)
	{
		perror("HERCULES_ERR_RELEASE_IMSS_FIND_IMSS");
		slog_error("HERCULES_ERR_RELEASE_IMSS_FIND_IMSS");
		pthread_mutex_unlock(&lock_network);
		return -1;
	}
...
/**
 * @brief Method retrieving information related to a certain IMSS instance.
 * This function only connects to the metadata server when the IMSS instance
 * is not locally.
 *
 * @param imss_info_ pointer to the info structure.
 * @return if the "info" is found locally, 2 is returned.
 * When a connection is perform to the metadata server,
 * 1 is returned on success, 0 if not found. On other case
 * -1 is returned.
 */
...
int32_t stat_imss_info()
{
	...
	if ((imss_found_in = find_imss(curr_imss.info.uri_, &local_imss_)) == -1)
	{
		perror("HERCULES_ERR_STAT_IMSS_INFO_FIND_IMSS");
		slog_error("HERCULES_ERR_STAT_IMSS_INFO_FIND_IMSS");
		return -1;
	}
	// get the pointer to modify it.
	imss *imss_;
	imss_ = &g_array_index(imssd, imss, imss_found_in);
	...
	int first_parent_offset = find_first_parent_dir(curr_imss.info.uri_, first_parent_dir);
	slog_debug("imss_uri=%s, first_parent_dir=%s, first_parent_offset=%d", curr_imss.info.uri_, first_parent_dir, first_parent_offset);
	// if (first_parent_offset > 0)
	// {
	m_srv = find_server(n_stat_servers, 0, first_parent_dir, GET, TYPE_METADATA_SERVER, curr_imss.info.session_plcy);
	...
	pthread_mutex_lock(&lock_network);
	// Send the request.
	sprintf(formated_uri, "%" PRIu32 " GET 0 %s", IMSS_INFO, curr_imss.info.uri_);
	...
	// imss_info_ = (imss_info *)request;
	imss_info *received_info_struct = (imss_info *)request;
	imss_info curr_info_struct = curr_imss.info;

	// TODO: compare the address of the info received and remove the elements and endpoints that does not match.
	for (size_t i = 0; i < curr_info_struct.num_storages; i++)
	{
		int found = 0;
		for (size_t j = 0; i < received_info_struct->num_storages; j++)
		{
			if (!strcmp(curr_info_struct.ips[i], received_info_struct->ips[j]))
			{ // current ip address found on the received struct.
				found = 1;
				break;
			}
		}
		int server_id_to_remove = i;
		size_t num_elements_to_shift = imss_->info.num_storages - server_id_to_remove - 1;
		// sort the ips array.
		memmove(&imss_->info.ips[server_id_to_remove],
			&imss_->info.ips[server_id_to_remove + 1],
	...
	// memcpy(&curr_imss.info, request, sizeof(imss_info));
	// free(request);
	pthread_mutex_unlock(&lock_network);
	return 1;
}
		       ...
		       char file_type)
{
	...
	{ // "dataset_uri" is not on the HERCULES root.
		...
		if (ret < 0 && ret != -3)
		{
			fprintf(stderr, "Any other error (%d) ocurred while seeking for parent directory %s, required by %s", ret, parent_dir, dataset_uri);
			return -ENOENT;
		}
	...
	{ // 1: The dataset exists in the local GArray.
		...
		{ // the dataset is in the local GArray, we have to clear the intervals.
	...
	new_dataset = (dataset_info *)calloc(1, sizeof(dataset_info));
	// new_dataset = (dataset_info *)malloc(sizeof(dataset_info));
	// new_dataset = (dataset_info *)calloc(1, sizeof(dataset_info));
	...
	// 	uint32_t info_size = 2000 * sizeof(uint16_t);
	// 	slog_info("info_size=%d, sizeof(uint16_t)=%d", info_size, sizeof(uint16_t));
	...
	// 	slog_info("new_dataset->data_locations address = %p", new_dataset->data_locations);
	...
	{ // this can be also a directory case but the uri already contains the last slash.
		sprintf(formated_uri, "%" PRIu32 " SET %lu %s", opened, msg_size, new_dataset->uri_);
	}
	...
	// add_dataset_entry(&datasetd, dataset_uri, aux_dataset);
	// init the hash table for intervals.
	add_dataset_entry_in_pool(dataset_uri, new_dataset);
...
/**
 * @brief  Method creating the required resources in order to READ and WRITE an existing dataset.
 ...
 * @return If already stored, position where the dataset was stored on the local map "datasetd" is returned,
 * if not, position where the dataset is inserted on the local map "datasetd" is returned, on error -1 is returned
 * and -2 for symbolic links.
 */
int32_t open_dataset(char *dataset_uri, int opened)
{
	...
	{ // link case.
		slog_debug("is_link=%d, link name=%s", new_dataset->is_link, new_dataset->link);
		...
		if (strncmp(new_dataset->link, "imss://", strlen("imss://")))
		{
			...
			slog_debug("No Hercules link %s", new_dataset->link);
	...
	{
		slog_debug("is_link=%d, stat_dataset_res=%d, new_dataset->local_conn=%d, n_servers=%d, name=%s, num_intervals=%d", new_dataset->is_link, stat_dataset_res, new_dataset->local_conn, new_dataset->n_servers, new_dataset->original_name, new_dataset->num_intervals);
	}
	int32_t not_initialized = 0;
	...
	default:
	{
		if (new_dataset->local_conn != -2)
		{
			...
			// 	{
			// 		slog_live("i=%d, dataset_uri=%s, dataset_info_->uri_=%s", i, dataset_uri, dataset_info_.uri_);
...
int32_t parse_malleability_message(void *result, const char *failed_hostname)
{
	if (!result)
	{
		slog_error("HERCULES_ERR_PARSE_MALLEABILITY_MESSAGE: Empty message");
		fprintf(stderr, "HERCULES_ERR_PARSE_MALLEABILITY_MESSAGE: Empty message");
		return 0;
	}
	// Get the pointer to the imss struct.
	imss *local_imss_;
	int32_t imss_position = -1;
	if ((imss_position = find_imss_pointer("imss://", &local_imss_)) == -1)
	{
		perror("HERCULES_ERR_PARSE_MALLEABILITY_MESSAGE_FIND_IMSS_POINTER");
		slog_error("HERCULES_ERR_PARSE_MALLEABILITY_MESSAGE_FIND_IMSS_POINTER");
		return -1;
	}

	int32_t changes_found = 0;
	...
	sscanf((const char *)result, "%s %" PRId32 " %" PRId32 " %s",
	       message, &new_number_of_data_servers, &id_modified_server, list_of_active_nodes);

	slog_debug("message=%s, new_number_of_data_servers=%" PRId32 ", id_modified_server=%" PRId32 ", list_of_active_nodes=%s",
		   message, new_number_of_data_servers, id_modified_server, list_of_active_nodes);

	// parse the expected ordered list from the metadata server
	char *expected_server_list[MAX_SERVERS] = {NULL};
	int size_expected_server_list = 0;

	char list_copy[PATH_MAX];
	strncpy(list_copy, list_of_active_nodes, PATH_MAX - 1);
	list_copy[PATH_MAX - 1] = '\0';

	char *token = strtok(list_copy, ",");
	while (token != NULL && size_expected_server_list < MAX_SERVERS)
	{
		expected_server_list[size_expected_server_list++] = token;
		token = strtok(NULL, ",");
	}

	if (size_expected_server_list == 0)
	{
		slog_error("parse_malleability_message: empty server list received");
		return 0;
	}

	// remove servers no longer in the expected list (decommission)
	int initial_count = (int)local_imss_->info.num_storages;
	for (int i = initial_count - 1; i >= 0; i--)
	{
		int still_active = 0;
		for (int j = 0; j < size_expected_server_list; j++)
		{
			if (!strcmp(local_imss_->info.ips[i], expected_server_list[j]))
			{
				still_active = 1;
				break;
			}
		...
		if (!still_active)
		{
			slog_debug("Server %s (id=%d) decommissioned, releasing.", local_imss_->info.ips[i], i);
	...
	// check slot by slot against the expected order
	for (int slot = 0; slot < size_expected_server_list; slot++)
	{
		int slot_exists = ((int)local_imss_->info.num_storages > slot);
		int slot_matches = slot_exists && (strcmp(local_imss_->info.ips[slot], expected_server_list[slot]) == 0);

		/*
		 * A slot need to be refresh when:
		 *  it mismatches the expected order from the received list, or
		 *  it matches by name but is the server that just failed
		 *  before calling this function (failed_hostname); endpoint must be recreated.
		 */
		int is_failed_server = (failed_hostname != NULL) && slot_exists && (strcmp(local_imss_->info.ips[slot], failed_hostname) == 0);

		if (slot_matches && !is_failed_server)
		{
			slog_debug("Slot %d OK: '%s'", slot, local_imss_->info.ips[slot]);
			continue;
		}
	...
	slog_debug("parse_malleability_message done, changes_found=%d, final num_storages=%d",
...
int32_t wait_malleability_changes(ucp_worker_h ucp_worker, uint64_t local_data_uid, ucp_ep_h ep, const char *failed_hostname)
{
	...
	// int id_modified_server, new_number_of_data_servers;
	changes_found = parse_malleability_message(result, failed_hostname);
...
int32_t get_malleability_changes(const char *dataset_uri, const char *failed_hostname)
{
	...
	int changes_found = wait_malleability_changes(ucp_worker_meta, local_meta_uid, ep, failed_hostname);
	slog_debug("changes_found=%d", changes_found);
...
int32_t close_dataset(const char *dataset_uri, int fd)
{
	...
	// slog_debug("[IMSS] before get_recv_data_length");
	msg_length = get_recv_data_length(ucp_worker_meta, local_meta_uid);
...
int32_t unlink_dataset(const char *dataset_uri, int32_t dataset_id, int deep)
{
	...
	for (int32_t i = 0; i < replication_factor; i++)
	{
		...
		// char *node_hostname = curr_imss.info.ips[server_id];
		char node_hostname[512];
		...
		slog_debug("Request to data %d (%s) - %s", i, node_hostname, key_);
		...
		{
			pthread_mutex_unlock(&lock_network);
			perror("HERCULES_ERR_UNLINK_DATASET_INVALID_MSG_LENGTH");
			slog_error("HERCULES_ERR_UNLINK_DATASET_INVALID_MSG_LENGTH");
			return -1;
		}
...
int32_t delete_dataset(const char *dataset_uri, int32_t dataset_id, int is_dir)
{
	...
	size_t len_dataset_uri = strlen(dataset_uri);

	// sprintf(formated_uri, "%" PRIu32 " GET 4 %s", stat_ids[m_srv], dataset_uri); // delete
	// delete OP
	if (is_dir)
	{ // directory.
		// check if the last slash is missing (required by the metadata server to identify directories).
		if (dataset_uri[len_dataset_uri - 1] != '/')
		{
			sprintf(formated_uri, "%d GET %d %s/", 4, DELETE_OP, dataset_uri);
		}
		else
		{
			sprintf(formated_uri, "%d GET %d %s", 4, DELETE_OP, dataset_uri);
	...
	{
		pthread_mutex_unlock(&lock_network);
		perror("HERCULES_ERR_DEL_DATASET_INVALID_MSG_LENGTH");
		slog_error("HERCULES_ERR_DEL_DATASET_INVALID_MSG_LENGTH");
		return -1;
	}
	...
	{
		pthread_mutex_unlock(&lock_network);
		perror("HERCULES_ERR_DEL_DATASET_RECV_DATA");
		slog_error("HERCULES_ERR_DEL_DATASET_RECV_DATA");
		free(result);
		return -1;
	}
	pthread_mutex_unlock(&lock_network);
...
int32_t rename_dataset_metadata_dir_dir(char *old_dir, char *rdir_dest)
{
	...
	{
		pthread_mutex_unlock(&lock_network);
		perror("ERR_HERCULES_REN_DATASET_METADATA_DIR_DIR_INVALID_MSG_LENGTH");
	...
	{
		pthread_mutex_unlock(&lock_network);
		perror("ERR_HERCULES_REN_DATASET_METADATA_DIR_DIR_RECV_DATA");
		slog_error("ERR_HERCULES_REN_DATASET_METADATA_DIR_DIR_RECV_DATA");
		free(result);
		return -1;
	}
	pthread_mutex_unlock(&lock_network);
...
int32_t open_local_dataset(const char *dataset_uri, int opened)
{
	...
	slog_live("[IMSS] opened=%d", opened);
	// Formated dataset uri to be sent to the metadata server.
	char formated_uri[REQUEST_SIZE] = {0};
	...
	slog_live("[IMSS] Metadata Request - %s", formated_uri);
	// Send the request.
	size_t msg_len = 0;
...
// Returns true if paths are the same (with or without trailing '/')
int paths_equal(const char *a, const char *b)
{
	size_t len_a = strlen(a);
	size_t len_b = strlen(b);

	// Fast path: exact match (including both with or without '/')
	if (strcmp(a, b) == 0)
	{
		return 1;
	}

	// Check if one is the other + '/'
	if (len_a == len_b + 1 && a[len_a - 1] == '/' && strncmp(a, b, len_b) == 0)
	{
		return 1;
	}
	if (len_b == len_a + 1 && b[len_b - 1] == '/' && strncmp(b, a, len_a) == 0)
	{
		return 1;
	}
...
int32_t stat_dataset(const char *dataset_uri, dataset_info **dataset_info_, int opened)
{
	...
	// if (g_hash_table_size(datasetd) > 0)
	{
		...
		// ret = find_dataset_by_uri_hash(datasetd, dataset_uri, &dataset_info_aux);
		ret = find_dataset_by_uri_hash_pool(dataset_uri, &dataset_info_aux);
...
int32_t get_data_location(char *dataset_uri, int32_t dataset_id, int32_t data_id, int32_t op_type)
{
	...
	// if (id_current_dataset != dataset_id) // TO FIX: if a thread does not met the condition, their curr_datset could have the value of the previous thread. curr_dataset is global.
	{
		...
		dataset_info *dataset_info_aux = NULL; // Currently managed dataset.
		...
		// curr_dataset = aux_dataset;
		// memcpy(&curr_dataset, dataset_info_aux, sizeof(dataset_info));
		curr_dataset = dataset_info_aux;

		// curr_imss = g_array_index(imssd, imss, curr_dataset->imss_d);
		curr_imss = g_array_index(imssd, imss, curr_dataset->imss_d);
		// slog_debug("Intervals add: %p", curr_dataset->intervals);
		...
		if (set_policy_dataset(&curr_dataset) == -1)
		{
	...
	slog_live("curr_dataset->uri=%s,  curr_dataset->original_name=%s, curr_imss.info.num_storages(%p)=%d, curr_dataset->n_servers=%d, curr_imss.info.num_active_storages=%d, curr_dataset->n_servers_when_created=%d", curr_dataset->uri_, curr_dataset->original_name, &curr_imss.info.num_storages, curr_imss.info.num_storages, curr_dataset->n_servers, curr_imss.info.num_active_storages, curr_dataset->n_servers_when_created);
	...
	{
		num_storages = entry_value; // entry->value;
	}
	slog_debug("num_storages to find server=%d", num_storages);
	...
	// if (curr_imss.info.num_active_storages > 1)
	{
		...
		// int num_storages = curr_imss.info.num_active_storages;
		if ((server = find_server(num_storages, data_id, curr_dataset->original_name, op_type, TYPE_DATA_SERVER, curr_dataset->session_plcy)) < 0)
		{
					  ...
					  int32_t dataset_id, int32_t data_id)
{
	...
	// Request the concerned block to the involved servers.
	// This is a special case where we need to send the request to all servers to
	// update the uri of all the subfiles in the old directory.
	// For example:
	// Server 0 holds the block 0 imss://dir/$0 and,
	// Server 1 holds the block 0 imss://dir/file.txt$0
...
int32_t rename_dataset_srv_worker(char *old_dataset_uri, char *new_dataset_uri, int32_t dataset_id, int32_t data_id)
{
	...
	for (int32_t i = 0; i < curr_dataset->repl_factor; i++)
	{
		...
		{
			pthread_mutex_unlock(&lock_network);
			perror("ERR_HERCULES_REN_DATASET_DATA_INVALID_MSG_LENGTH");
			slog_error("ERR_HERCULES_REN_DATASET_DATA_INVALID_MSG_LENGTH");
			return -1;
		}
...
/**
 * @brief  Method to delete the dataset data on the backend data.
 * @return 0 if the file was correctly delete, on error -1 is returned.
 */
		       ...
		       int64_t size)
{
	...
	for (int32_t i = 0; i < curr_dataset->repl_factor; i++)
	{
		...
		if (send_req(ucp_worker_data, ep, local_addr_data, local_addr_len_data, key_) == 0)
		{
			...
			slog_error("ERR_HERCULES_RLSIMSS_SENDADDR");
			return -1;
		}
		...
		{
			pthread_mutex_unlock(&lock_network);
			perror("ERR_HERCULES_REN_DATASET_DATA_DIR_DIR_INVALID_MSG_LENGTH");
			slog_error("ERR_HERCULES_REN_DATASET_DATA_DIR_DIR_INVALID_MSG_LENGTH");
			return -1;
		}
...
void *split_readv(void *th_argv)
{
	...
	for (int32_t i = 0; i < curr_dataset->repl_factor; i++)
	{
		...
		{
			pthread_mutex_unlock(&lock_network);
			perror("ERR_HERCULES_SPLIT_READ_INVALID_MSG_LENGTH");
			slog_error("ERR_HERCULES_SPLIT_READ_INVALID_MSG_LENGTH");
			pthread_exit(NULL);
		}
		// to check!
		...
		if (msg_length == 0)
		{
			...
			{
				pthread_mutex_unlock(&lock_network);
				perror("ERR_HERCULES_SPLIT_READ_RECV");
				pthread_exit(NULL);
			}
			else
				break;
...
{
	slog_debug("Printing %d intervals, address=%p", curr_dataset->num_intervals, curr_dataset);
	for (size_t i = 0; i < curr_dataset->num_intervals; i++)
	{
...
void SetInterval(dataset_info *curr_dataset, int value, int left_interval, int right_interval)
{
	...
	{
		fprintf(stderr, "Current dataset is NULL\n");
		slog_error("Current dataset is NULL");
		return;
	}
...
ssize_t get_ndata(char *dataset_uri, int32_t dataset_id, int32_t data_id, void *buffer, ssize_t to_read, off_t offset, int async, void **buffer_request, int deep)
{
	...
	for (int32_t i = 0; i < replication_factor; i++)
	{
		...
		if (use_local)
		{
			...
			shm_id = getIdentifierSM(shm_key, to_read);
			// fprintf(stderr, "Using shm_id=%d of size %ld\n", shm_id, to_read);
			...
			sprintf(key_, "%s %lu %ld %d %s$%d %ld", mode, 0l, offset, shm_id, curr_dataset->uri_, data_id, to_read);
		}
		else
		{
			sprintf(mode, "GET");
		...
		else
		{
			...
			if (use_local)
			{
				...
				// copyContentSM(buffer, sh_memory_struct->content, to_read);
				copyContentSM(buffer, content, to_read);
				// detach shared memory.
...
{
	dataset_info *dataset_info_aux = NULL; // Currently managed dataset.
	int32_t ret = find_dataset_by_uri_hash_pool(dataset_uri, &dataset_info_aux);
	if (ret == 0)
	{
		slog_error("Uri not found:%s", dataset_uri);
		return -1;
	...
	if (dataset_info_aux == NULL)
	{
		perror("HERCULES_ERR_UPDATE_DATASET_URI_NOT_STORED");
		return -1;
	}
	...
	{ // "opened==2" indicates a directory case (see mkdir) and checks if the last slash is missing (mandatory for the metadata server).
		// add the last slash.
		sprintf(formated_uri, "%" PRIu32 " SET %lu %s/", opened, msg_size, curr_dataset->uri_);
	}
	else
	{ // this can be also a directory case but the uri already contains the last slash.
		sprintf(formated_uri, "%" PRIu32 " SET %lu %s", opened, msg_size, curr_dataset->uri_);
...
int32_t wait_ack_set_data(char **response_buffer, ucp_ep_h ep, int async)
{
	// wait for response.
	// ack from the server.
	...
	*response_buffer = (char *)malloc(msg_length * sizeof(char));
	if (*response_buffer == NULL)
	{
	...
	return (int32_t)size_received_data;
}
...
int32_t set_data(char *dataset_uri, int32_t dataset_id, int32_t data_id, const void *buffer, size_t size, off_t offset, int async, int deep, hercules_modes_t op_type)
{
	...
	for (int32_t i = 0; i < curr_dataset->repl_factor; i++)
	{
		...
		strncpy(node_hostname, curr_imss.info.ips[server_id], 512);
		client_ep_context_t *this_context = curr_imss.conns.ep_contexts[n_server_];
		if (this_context != NULL)
		{
			this_context->status = UCS_OK;
		}
		...
		if (use_local)
		{
			...
			if (shm_id == -1)
			{
				slog_error("HERCULES_ERR_SET_DATA_GET_IDENTIFIER_INVALID_SHM_ID: key %d size %zu", shm_key, size);
				pthread_mutex_unlock(&lock_network);
				return -ECANCELED;
			}
			...
			// Receive confirmation from the server to detach the memory.
			msg_length = TIMING(get_recv_data_length_with_cb(ucp_worker_data, local_data_uid, this_context), "get_recv_data_length", size_t, process_rank);
			if (msg_length == (size_t)-1)
			{
			...
			if (msg_length > 0)
			{
				void *response_buffer = malloc(msg_length);
				recv_data(ucp_worker_data, ep, response_buffer, msg_length, local_data_uid, async);
				free(response_buffer);
			}

			unlinkSM(content);
			slog_info("[IMSS] Local block has been processed");
		}
		else
		{ // non LOCAL policy.
			...
			{
				// Receive ACK with fault tolerance checks
				msg_length = TIMING(get_recv_data_length_with_cb(ucp_worker_data, local_data_uid, this_context), "get_recv_data_length", size_t, process_rank);
				if (msg_length == (size_t)-1)
				{
				...
				char *response_buffer = (char *)malloc(msg_length + 1);
				recv_data(ucp_worker_data, ep, response_buffer, msg_length, local_data_uid, async);
				response_buffer[msg_length] = '\0';
		   ...
		   int32_t *num_storages)
{
	...
	if ((server = find_server(where_imss.info.num_storages, data_id, where_dataset->uri_, GET, TYPE_DATA_SERVER, where_dataset->session_plcy)) < 0)
	{
...
/**
 * @brief C function to concat last slash to "pathname" if it does not have it.
 * @param pathname char pointer to the string.
 * @return 0 if "pathname" already contains the last slash, 1 if it was added.
 */
...
/**
 * @brief C function to remove last slash to "pathname" if it have it.
 * @param pathname char pointer to the string.
 * @return 0 if "pathname" does not contain the last slash, 1 if it was removed.
 */
int RemoveLastSlashC(char *pathname)
{
	...
	if (len > 0 && pathname[len - 1] == '/')
	{
		slog_debug("Removing last slash from %s", pathname);
		pathname[len - 1] = '\0';
		ret = 1;
	}
	return ret;
...
/**
 * @brief C++ function to concat last slash to "pathname" if it does not have it.
 * @param pathname string type concat the slash.
 * @return 0 if "pathname" already contains the last slash, 1 if it was added.
 */
...
/**
 * @brief C++ function to remove last slash to "pathname" if it have it.
 * @param pathname string type pointer to the string.
 * @return 0 if "pathname" does not contain the last slash, 1 if it was removed.
 */
...
char get_type(const char *uri)
{
	...
	char formated_uri[REQUEST_SIZE] = {0};
	char first_parent_dir[URI_] = {0};
	char ret_data_type = TYPE_ERROR;
	int32_t recv_msg_len = 0;

	// In case of multiple metadata servers, we use the first parent directory as name to
	// locate the block.
	find_first_parent_dir(uri, first_parent_dir);
	int is_root_path = 0; // =
	if (!strncmp(uri, "imss://", strlen(uri)))
	{
		is_root_path = 1;
	}

	slog_debug("uri=%s, first_parent_dir=%s, not_root=%d", uri, first_parent_dir, is_root_path);
	...
	if (m_srv < 0)
	{
		perror("HERCULES_ERR_GET_TYPE_INVALID_SRV");
		slog_error("HERCULES_ERR_GET_TYPE_INVALID_SRV");
		return TYPE_ERROR;
	}
	...
	slog_info("Request to metadata %d - '%s'", m_srv, formated_uri);
	...
	if (result == NULL)
	{
		perror("HERCULES_ERR_GET_TYPE_MEMORY_ALLOC");
	...
	if (recv_msg_len < 0)
	{
		slog_warn("HERCULES_ERR_GET_TYPE_RECV_STREAM");
		free(result);
		pthread_mutex_unlock(&lock_network);
		return TYPE_ERROR;
	}
	...
	{ // another dataset.
		...
		dataset_info *dataset = (dataset_info *)result;
		// ret_data_type = ((dataset_info *)result)->type;
		ret_data_type = dataset->type;
		// Add the created struture into the underlying IMSSs.
		// int32_t ret = GInsert(&datasetd_pos, &datasetd_max_size, (char *)&new_dataset, datasetd, free_datasetd);
		dataset_info *aux_dataset = (dataset_info *)malloc(sizeof(dataset_info));
	...
	{ // root case (not stored as dataset, but imss_info).
		ret_data_type = ((imss_info *)result)->type;
	}

	pthread_mutex_unlock(&lock_network);

	slog_debug("data->type=%c", ret_data_type);
